Заранее импортируем пару полезных функций, чтобы сработало всё, что ниже.
...
строки (значения типа `String`) в лямбда-выражения (значения типа `Exp`).
...
Зависит от стуктуры выражения:

> eval(exp) = case exp of

Главное правило — аппликация абстракции.
Если функция задана абстракцией, то результат применения её к аргументу —
это подстановка этого аргумента в тело этой абстракции вместо имени переменной,
заданной в абстракции.
